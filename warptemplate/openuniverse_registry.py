...
BASE_DIR = '/Users/jnordin/data/openUniverse24/MODELS-1_TRANSIENT_SED'
THRESHOLD = 0.01

# Cache-Verzeichnis für serialisierte Source-Objekte
_CACHE_DIR = os.path.join(os.path.expanduser('~'), '.cache', 'sncosmo_openuniverse')
...
def plot_diagnostics(name, base_dir=BASE_DIR):
    ...
    pivoted = pd.DataFrame(flux, index=phase, columns=wave)
    ...
    axes[0, 0].set_xlabel('phase'); axes[0, 0].set_ylabel('max |flux|')
    ...
    axes[0, 1].set_xlabel('wavelength'); axes[0, 1].set_ylabel('max |flux|')

    # ZTF light curves
    source = sncosmo.TimeSeriesSource(phase, wave, flux, name=name)
    axes[1, 0].plot(phase, source.bandflux('ztfg', phase))
    axes[1, 0].set_xlabel('phase'); axes[1, 0].set_ylabel('ztfg flux')

    axes[1, 1].plot(phase, source.bandflux('ztfi', phase))
    axes[1, 1].set_xlabel('phase'); axes[1, 1].set_ylabel('ztfi flux')
